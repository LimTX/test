...
class Cart extends AppModel
{
	...
	*/	
	function getCart($pid, $sid,$newCartkeyAct) {		
		return $this->find( 'all', array(
		'conditions'=>array('Cart.product_id'=>$pid, 'Cart.ct_session_id'=>$sid,'Cart.keypass'=> $newCartkeyAct),
		'order' => 'Cart.id DESC' )
		);
	}
	...
	*/
	...
	function getCartContent( $sid ) {
		$cartContent = array();
		$sql = "SELECT ct.id, ct.product_id, ct.qty, pd.name, pd.description, pd.price, pd.image, pd.category_id
			FROM carts ct, products pd, categories cat
			WHERE ct_session_id = '$sid' AND ct.process = 'UnPaid' AND ct.product_id = pd.id AND cat.id = pd.category_id";	
	...
	function getCartContentNum( $sid ) {
		$cartContent = array();
		$sql = "SELECT ct.id, ct.process, ct.product_id, ct.qty, pd.name, pd.description, pd.price, pd.thumbnail, pd.category_id
			FROM carts ct, products pd, categories cat
			WHERE ct_session_id = '$sid' AND ct.process = 'UnPaid' AND ct.product_id = pd.id AND cat.id = pd.category_id";	
		$results = $this->query( $sql );	
		foreach ($results as $result ) {	
