...
$('#plus').toggle(function() {
    $('#plus').css("backgroundImage",'url("../img/minus.jpg")');
	$('.beta-content').hide();
	$('.featuredtable').fadeIn(200);
	$('.alpha-content').animate({"width": "92%",}, 500 );
}, function() {
    $('#plus').css("backgroundImage",'url("../img/plus.jpg")');
	$('.alpha-content').animate({"width": "62%",}, 500 );
	$('.beta-content').delay(700).fadeIn();
	$('.featuredtable').fadeOut(200);
});
...
<body>    
       ...
       <div class="beta" style="width:360px;">
               <div class="loginTop">
            <ul>
	...
	<?php echo $html->link('( Log out )', array('admin'=> false,'controller'=>'customers','action'=>'logout'), array('class' => 'toplinking')); ?>
    <?php echo $html->link(' My Account', array('admin'=> false,'controller'=>'customers','action'=>'account'), array('class' => 'toplinking'));  
	if($admin) { 
	    echo " |";  echo $html->link(' Admin Panel', array('admin'=> true,'controller'=>'products','action'=>'index'), array('class' => 'toplinking'));
	}
	?>
	<?php } else { ?>
    <?php echo $html->link('( Log in )', array('admin'=> false,'controller'=>'customers','action'=>'login'), array('class' => 'toplinking')); ?> <?php echo $html->link('( Register )', array('admin'=> false,'controller'=>'customers','action'=>'registration'), array('class' => 'toplinking')); ?> <?php echo $html->link(' My Account', array('admin'=> false,'controller'=>'customers','action'=>'account'), array('class' => 'toplinking')); } ?></li>
        ...
        <div style="padding-top:1px;">
          ...
          <div class="search_query">
		   <!--<input id="search_query_top" class="search_query" type="text" value="" name="search_query"> -->
			<?php echo $form->create(null, array('type' => 'get' ,'url' => array('admin'=> false,'controller' => 'products', 'action' => 'search'),'class'=>'genform','id'=>'searchbox2')); ?>
			<a href="javascript:document.getElementById('searchbox2').submit();">Go!</a>
<?php echo $form->input('search_text', array('id' =>'g','placeholder'=>'Search','label' => false)); ?><?php echo $form->end(); ?>
		 ...
		 <div class="loginTop2">
<?php echo $html->link('Advanced Search', array('admin'=> false,'controller'=>'products','action'=>'adv_search'), array('style' => ' text-align:right; border:0px solid #fff; ','class' => 'toplinking'));?>
