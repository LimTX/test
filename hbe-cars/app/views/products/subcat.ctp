...
?>
<p>HOME &gt; CATEGORIES &gt; <span style="text-transform:uppercase;">
<?php 

$categories = $this->requestAction("/categories/getProdCategoryName");
foreach($categories as $category) { 
 echo $category['Category']['name'];
}
?><?php $categories = $this->requestAction("/categories/getCategoryName");	
foreach($categories as $category) { ?>
</span> &gt; <span style="text-transform:uppercase;"><?php echo $category['Category']['name']; ?></span></p>
<br />
<div class="titleBar">
 ...
 <thead>
    	<tr>
          ...
          <div id="prodno"><?php echo $totalProducts." Result(s)"; ?></div>
            <div id="high">
            <?php echo $paginator->sort('Price (Highest to Lowest)', 'price', array('price' => 'Price (Lowest to Highest)','class' => 'normalTip')); ?>
            </div>  
            <div id="low">
...
<div id="sideBar" style="padding-top:2px;">
 <div class="sfeaturepro">
            <div class="ribbon"></div>
            <div id="slider" class="nivoSlider">
          
  <?php foreach ($featuredproducts as $featuredproduct) { //Check if images exist and display images
    if ( $featuredproduct['Product']['image'] ) {
        $thumbnail = '/img/sets/big/' . $featuredproduct['Product']['image'];
    } else {
        $thumbnail = 'no-image-large.png';
    } 
		
      e( $html->link(
	      $html->image($thumbnail, array('width' => '542', 'height' => '350', 'border' => '0', 'title' => $featuredproduct['Product']['name']  )),
          array('controller' =>'products',
		  'action' => "view/pd_id:".$featuredproduct['Product']['id']
		  ), array('escape' => false))); ?>
<?php } ?>        
