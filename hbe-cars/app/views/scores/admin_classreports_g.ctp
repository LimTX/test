    ...
    $(document).ready(function() {	   				
       chart = new Highcharts.Chart({
         ...
         series: [{
			...
			?>
            data: [
			<?php  if ($glessontotal1 != 0) { $glesson1 = $MIGraphResp1 + $FDGraphResp1 + $USGraphResp1 + $CEGraphResp1 +  $CCGraphResp1 + $MPGraphResp1 + $WMGraphResp1 + $CIGraphResp1 + $FOGraphResp1 + $APGraphResp1 + $FLGraphResp1+ $SMGraphResp1; echo $glesson1total = round($glesson1 / $glessontotal1, 2) / 0.01; } else { echo $glesson1total = 0 ; } ?>,
...
<h4 class="alert_info">Book G &nbsp;&nbsp; CARS &reg; Series</h4>
<br/><br/>
                    ...
                    <th class="header"><?php echo $html->link('FO', '#', array('title' => 'Distinguishing Between Fact and Opinion','style'=> 'color:#333;font-size:9px;')); ?></th>
                     <th class="header"><?php echo $html->link('AP', '#', array('title' => 'Identifying Author\'s Purpose','style'=> 'color:#333;font-size:9px;')); ?></th>
                      <th class="header"><?php echo $html->link('FL', '#', array('title' => 'Interpreting Figurative Language','style'=> 'color:#333;font-size:9px;')); ?></th>
                       <th class="header"><?php echo $html->link('SM', '#', array('title' => 'Summarising','style'=> 'color:#333;font-size:9px;')); ?></th>
				</tr>
              </thead>
              <tbody> 
               <?php foreach ($customers as $customer) { ?>
				<tr> 
                    ...
                    <td class="nobold"<?php $perlesson = $customer['Score']['MI_pts'] + $customer['Score']['FD_pts'] + $customer['Score']['US_pts'] + $customer['Score']['CE_pts'] + $customer['Score']['CC_pts'] + $customer['Score']['MP_pts'] + $customer['Score']['WM_pts'] + $customer['Score']['CI_pts'] + $customer['Score']['FO_pts'] + $customer['Score']['AP_pts'] + $customer['Score']['FL_pts'] + $customer['Score']['SM_pts'];
 ...
 <?php for ($i=1; $i<=10; $i++) { ?>
     ...
     <?php 	$lessonid = $i;
		...
		<div class="tab_container">
			<div id="tab1<?php echo $lessonid ?>" class="tab_content">
            <?php if($customers) { ?>
			<table class="tablesorter" cellspacing="0" style="font-family: 'Helvetica Neue', Helvetica, Arial, Verdana, sans-serif;">
            <thead>
            <tr> 
    				<th width="140" style="padding-right:5px;" class="empty"></th> 
                    <th width="120" class="header">No of Correct</th> 
