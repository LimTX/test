...
$checkscore = $this->requestAction('/scores/checkscorecompletedlesson/'.$access_token."/".$navlessonid);
if (count($checktest) == count($checkscore)) {
echo $html->link('BM 2', array('controller' => 'scores', 'action' => 'edit_carsplus_p_7', $this->params['pass'][0],$this->params['pass'][1],$this->params['pass'][2]),array('id' => 'full-qns','title' => 'Benchmark 2'));
} else {
echo $html->link('BM 2', array('controller' => 'scores', 'action' => 'edit_carsplus_p_7', $this->params['pass'][0],$this->params['pass'][1],$this->params['pass'][2]),array('title' => 'Benchmark 2'));
} ?> &nbsp; &bull; &nbsp;
<?php
$navlessonid = 8;
$checkscore = $this->requestAction('/scores/checkscorecompletedlesson/'.$access_token."/".$navlessonid);
...
<?php 
echo $html->link('PT 1', array('controller' => 'scores', 'action' => 'edit_carsplus_p_11', $this->params['pass'][0],$this->params['pass'][1],$this->params['pass'][2]),array('id' => 'active', 'title' => 'Post Test 1'));
?>  &nbsp; &bull; &nbsp;
<?php
$navlessonid = 12;
$checkscore = $this->requestAction('/scores/checkscorecompletedlesson/'.$access_token."/".$navlessonid);
...
<header><h6>Post Test 1</h6></header>
            
<?php
echo $form->create(null, array('url' => array('controller' => 'scores', 'action' => 'edit_carsplus_p_11'),'class'=>'cmxform','id'=>'form2','type' => 'file'));
...
echo $form->input('FO', array('label' => false,'class'=>'ProductStatusPublish','validate'=>'required:true','options' => $options,'legend' => false,'separator' => '&nbsp;&nbsp;', 'type' => 'radio'));
 ?><label for="data[Score][FO]" class="error">This field is required.</label>
    ...
    <?php
	 echo $form->hidden('lesson', array('value' => 11));
